...
describe("微信品专广告", () => {
  ...
  beforeAll(async () => {
    await superView(6404911937, "wxid_3v5oigo9a2c212");
    pageExtend = await setup("wxadtestVidFeeds", 20, 3192187635, true);
  ...
  test("> q=wxadtestVidFeeds，验证混排结果页品专广告是否召回", async () => {
    await addMsg({
      context: undefined,
  ...
  // openFinderView
  test("> 点击广告头部，验证是否正确跳转到视频号动态落地页", async () => {
    await addMsg({
      context: undefined,
      message: ` 测试步骤：\n  1. 输入搜索query=wxadtestVidFeeds,发起搜索\n  2. 点击品专广告头部区域，检查跳转目标为广告原生页，校验canvansid`
    });
    let num = 3;
    while(num != 0){
      try  {
        ...
        expect(pageExtend.extendInfo).toBe("export/UzFfAgtgekIEAQAAAAAA1R8WTk29QQAAAAstQy6ubaLX4KHWvLEZgBPEjKMwLzwHbJiDzNPgMIxuNThPtRo3XbJsPlkk2LxL");
        break;
      }catch (e) {
        if (num == 1){
          if (e.constructor.name == "JestAssertionError"){
  ...
  test("> 验证\"进入视频号 \"外链文案是否正确；点击外链，验证是否正确跳转到视频号动态落地页", async () => {
    await addMsg({
      context: undefined,
      message: ` 测试步骤：\n  1. 输入搜索query=wxadtestVidFeeds,发起搜索\n  2. 检查广告头部展示"进入视频号"外链\n  3. 点击"进入视频号"，检查跳转目标为"WXAD测试号"视频号`
    });
    let num = 3;
    while(num != 0){
      try  {
  ...
  test("> 点击广告名称，验证是否正确跳转到\"WXAD测试号\"视频号", async () => {
    await addMsg({
      context: undefined,
      message: ` 测试步骤：\n  1. 输入搜索query=wxadtestVidFeeds,发起搜索\n  2. 点击广告名称区域，检查跳转目标为"WXAD测试号"视频号`
    });
    let num = 3;
    while(num != 0){
      try  {
  ...
  test("> 点击在线客服按钮，验证跳转链接配置是否正确", async () => {
    ...
    while(num != 0){
      try  {
        ...
        const image =  await ele.screenshot({path: path});
        await addAttach({attach: image, description: "客服按钮"});
  ...
  test("> 点击联系电话按钮，验证是否正确展示弹窗及对应联系电话", async () => {
    ...
    while(num != 0){
      try  {
        ...
        let phoneArr = ["17000001688", "17000001689"];
  ...
  test("> 验证广告视频号账号信息展示正确；点击账号，验证是否正确跳转到\"WXAD测试号\"视频号", async () => {
    await addMsg({
      context: undefined,
  ...
  test("> 关注视频号账号，验证账号\"已关注\"标签是否正常显示", async () => {
    await addMsg({
      context: undefined,
  ...
  test("> 验证广告小程序账号信息展示正确；点击账号，验证是否正确跳转到\"苏宁拼购\"小程序", async () => {
    await addMsg({
      context: undefined,
      message: ` 测试步骤：\n  1. 输入搜索query=wxadtestVidFeeds,发起搜索\n  2. 查看视频号账号信息，标题为"苏宁拼购"，底部显示"小程序"\n  3. 点击视频号账号主体，检查跳转目标为\"苏宁拼购\"小程序`
    });
    let num = 3;
    while(num != 0){
      try  {
        ...
        let ele =  await page.$$(wxAdClass.account);
        let path = './static/pic/ad_weapp.png';
        const image =  await ele.at(1).screenshot({path: path});
        await addAttach({attach: image, description: "账号截图"});
        ...
        expect(content[0]).toBe("苏宁拼购");
        expect(content[1]).toBe("小程序");

        await page.click(className);
        await page.waitForTimeout(700);
        expect(pageExtend.extendInfo).toBe("gh_157a722e7318@app");
        break;
      }catch (e) {
        if (num == 1){
          if (e.constructor.name == "JestAssertionError"){
  ...
  test("> 点击热门活动1，验证是否正确跳转到\"唯品会特卖\"小程序", async () => {
    await addMsg({
      context: undefined,
  ...
  test("> 点击热门活动2，验证是否正确跳转到\"苏宁拼购\"小程序", async () => {
    await addMsg({
      context: undefined,
      message: ` 测试步骤：\n  1. 输入搜索query=wxadtestVidFeeds,发起搜索\n  2. 点击热门活动第2个活动，检查跳转目标为"苏宁拼购"小程序`
    });
    let num = 3;
    while(num != 0){
      try  {
  ...
  test("> 点击热门活动3，验证是否正确跳转到广告原生页", async () => {
    await addMsg({
      context: undefined,
      message: ` 测试步骤：\n  1. 输入搜索query=wxadtestVidFeeds,发起搜索\n  2. 点击热门活动第3个活动，检查跳转目标为广告原生页，校验cavansid`
    });
    let num = 3;
    while(num != 0){
      try  {
        ...
        await page.waitForTimeout(700);
        expect(pageExtend.extendInfo).toBe("2580045070");
        break;
      }catch (e) {
        if (num == 1){
          if (e.constructor.name == "JestAssertionError"){
  ...
  test("> 点击热门活动3左侧菜单，验证是否正确跳转到\"唯品会特卖\"小程序", async () => {
    await addMsg({
      context: undefined,
  ...
  test("> 点击热门活动3右侧菜单，验证是否正确跳转到百度首页", async () => {
    await addMsg({
      context: undefined,
      message: ` 测试步骤：\n  1. 输入搜索query=wxadtestVidFeeds,发起搜索\n  2. 点击热门活动第3个活动右侧菜单，检查跳转目标为H5页面（百度首页）`
    });
    let num = 3;
    while(num != 0){
      try  {
  ...
  test("> 点击热门活动4，验证是否正确跳转到\"鹅房通\"小程序", async () => {
    await addMsg({
      context: undefined,
      message: ` 测试步骤：\n  1. 输入搜索query=wxadtestVidFeeds,发起搜索\n  2. 点击热门活动第4个活动，检查跳转目标为"鹅房通"小程序`
    });
    let num = 3;
    while(num != 0){
      try  {
        ...
        await page.waitForTimeout(700);
        expect(pageExtend.extendInfo).toBe("gh_ee0415ce7cf3@app");
        break;
      }catch (e) {
        if (num == 1){
          if (e.constructor.name == "JestAssertionError"){
  ...
  test("> 点击热门活动4左侧菜单，验证是否正确跳转到广告原生页", async () => {
    await addMsg({
      context: undefined,
      message: ` 测试步骤：\n  1. 输入搜索query=wxadtestVidFeeds,发起搜索\n  2. 点击热门活动第4个活动左侧菜单，检查跳转目标为广告原生页，校验cavansid`
    });
    let num = 3;
    while(num != 0){
      try  {
        ...
        await page.waitForTimeout(700);
        expect(pageExtend.extendInfo).toBe("2580017336");
        break;
      }catch (e) {
        if (num == 1){
          if (e.constructor.name == "JestAssertionError"){
  ...
  test("> 点击热门活动4右侧菜单，验证是否正确跳转到百度首页", async () => {
    await addMsg({
      context: undefined,
      message: ` 测试步骤：\n  1. 输入搜索query=wxadtestVidFeeds,发起搜索\n  2. 点击热门活动第4个活动右侧菜单，检查跳转目标为H5页面（百度首页）`
    });
    let num = 3;
    while(num != 0){
      try  {
  ...
  test("> 点击热门活动5，验证是否正确跳转到广告原生页", async () => {
    await addMsg({
      context: undefined,
    ...
    while(num != 0){
      try  {
        ...
        await page.waitForTimeout(700);
        expect(pageExtend.extendInfo).toBe("2580033501");
        break;
      }catch (e) {
        if (num == 1){
          if (e.constructor.name == "JestAssertionError"){
  ...
  test("> 点击热门活动5行动按钮，验证是否正确跳转到广告原生页", async () => {
    await addMsg({
      context: undefined,
    ...
    while(num != 0){
      try  {
        ...
        expect(content.split("<!---->")[1]).toBe("领取福利");
