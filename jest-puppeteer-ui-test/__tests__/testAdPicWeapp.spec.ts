...
describe("微信商品品专广告测试 -- wxadtestPicWeapp", () => {
    ...
    beforeAll(async () => {
        await superView(6404904332, "wxid_4je5lhahfpo12");
        pageExtend = await setup("wxadtestPicWeapp", 20, 3190188713);
    ...
    test("测试广告账号（公众号账号）", async () => {
        try {
            await addMsg({context: undefined, message: `校验广告公众号账号"快乐测试123"，点击关注后显示"已关注"tag。`});
            await page.bringToFront();
            ...
            let image =  await ele.at(1).screenshot();
    ...
    test("测试产品系列tab点击", async () => {
        try {
            await addMsg({context: undefined, message: `测试系列产品点击正常，且当前系列下所有的产品个数，及落地页path不同。`});
            await page.bringToFront();
    ...
    test("测试更多 -- account_more", async () => {
        try {
            await addMsg({context: undefined, message: `测试更多账号，及落地页正常。`});
            await page.bringToFront();
