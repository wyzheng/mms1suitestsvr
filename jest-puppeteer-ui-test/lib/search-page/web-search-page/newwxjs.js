...
k=j.BufferedBlockAlgorithm=f.extend({reset:function(){this._data=new n.init;this._nDataBytes=0},_append:function(a){"string"==typeof a&&(a=r.parse(a));this._data.concat(a);this._nDataBytes+=a.sigBytes},_process:function(a){var c=this._data,b=c.words,d=c.sigBytes,f=this.blockSize,h=d/(4*f),h=a?e.ceil(h):e.max((h|0)-this._minBufferSize,0);a=h*f;d=e.min(4*a,d);if(a){for(var g=0;g<a;g+=f)this._doProcessBlock(b,g);g=b.splice(0,a);c.sigBytes-=d}return new n.init(g,d)},clone:function(){var a=f.clone.call(this);
    ...
    var _sendMessageQueue = [], // page to native的消息队列
        ...
        _call_function_datas = [], // 保存调用call方法的参数以及callBack的数组,object.params,obejct.callback,object.func
    ...
    function _sendMessage(message) {
            if (true /* __ISWKWEBVIEW */) {
                //先使用fechQueue方法获取消息签名
                _sendMessageQueue.push(message);
                var fetchQueueMsg = _fetchQueue();
                //window.webkit.messageHandlers.weixinDispatchMessage.postMessage(fetchQueueMsg);
                // window.weixinPostMessageHandlers.weixinDispatchMessage.postMessage(fetchQueueMsg); //用提前注入的保护方法调用
                window.weixinDispatchMessage(fetchQueueMsg);
            } else {
                _sendMessageQueue.push(message);
    ...
    function _handleMessageFromWeixin(originmessage) {
        ...
        try {
            ...
            if (typeof window.weixinJSONParse !== 'undefined') {
                message = window.weixinJSONParse(messageStr);
            } else {
        ...
        case 'event':
            {
                ...
                //console.log('_EVENT_ID: ', message[_EVENT_ID])
    ...
    function _getImageDataAsBase64(inputImg, cb) {
        ...
        var _getBase64Impl = function (img) {
            ...
            if (img.src.match(re_data_url)) {
                dataURL = img.src;
            } else if (img.width * img.height < 0.6 * 1024 * 1024) { //过大的图片就不取base64了，会导致卡顿
    ...
    function _enable_mediaPlayChangeNotify() {
        ...
        for (var i = 0; i < audioElements.length; i++) {
            if (!audioElements[i].paused && !audioElements[i].ended) {
    ...
    function _setDefaultEventHandlers() {
        ...
        _on('menu:setfont', function (argv) {
            ...
            // ipados13的-webkit-text-size-adjust无效了，单独处理
            if (window.__wxjs_ipadfontsolution && _WXJS.os.ipad && parseFloat(_WXJS.os.version, '10') >= 13) {
              var root = document.getElementsByTagName('body')[0];
              var percent = parseFloat(s, '10') / 100;
              var setQueue = [];
              ...
              while (textWalker.nextNode()) {
                ...
                // 先不要马上设置fontSize，会影响子节点的fontSize，存起来最后统一设置
                setQueue.push([textContainer, oriFontSize]);
              }
              setQueue.forEach(function (setPair) {
                setPair[0].style.fontSize = parseFloat(setPair[1]) * percent + 'px';
              });
            }
        });
        ...
        _on('general:share', function (argv) {
            //console.log('进入默认 general:share 回调', argv)
            _log('general share');

            // 分享给朋友、收藏、企业微信、connector（不知道是什么）都会 emit 一个 menu:share:appmessage 事件
            if (argv['shareTo'] === 'friend' || argv['shareTo'] === 'favorite' || argv['shareTo'] === 'connector' || argv['shareTo'] === 'wework' || argv['shareTo'] === 'weworklocal') {
                {
                    //console.log('即将 emit "menu:share:appmessage"')
            ...
            if (argv['shareTo'] === 'QQ') {
                if (typeof _event_hook_map_for3rd['menu:share:qq'] === 'function' && _isIn3rdApiList('menu:share:qq')) {
                    //如果第三方有自定分享到qq或者qzone，就使用自定义function
                    _emit('menu:share:qq', argv);
                } else {
                    //分享到qq和qzone都使用appmessage，是为了统一第三方自定义分享数据
                    _emit('menu:share:appmessage', argv);
                }
                return;
            }
            ...
            if (argv['shareTo'] === 'QZone') {
                if (typeof _event_hook_map_for3rd['menu:share:QZone'] === 'function' && _isIn3rdApiList('menu:share:QZone')) {
                    //如果第三方有自定分享到qq或者qzone，就使用自定义function
            ...
            if (argv['shareTo'] === 'weread') {
                //分享到微信读书都使用appmessage，是为了统一第三方自定义分享数据
                _emit('menu:share:appmessage',argv);
                return;
            }
        });
        ...
        // 默认的分享给朋友、收藏、企业微信、 QQ、QQ 空间都会走到这里来
        var sendAppMessageFunc = function (argv) {
            _log('share appmessage');

            //console.log('调起默认 sendAppMessageFunc', argv)
            var data;
            if (typeof argv.title === 'string') { // 不知道这个分支是什么情况会进来？？？
                data = argv;
        ...
        _on('ui:longpress', function (argv) {
            _log("longpress at (" + argv.x + "," + argv.y + "," + argv.webViewWidth + "," + argv.webViewHeight + ")");
            var scale = document.body.clientWidth / argv.webViewWidth;
            var el = _getImageElementAtPoint(argv.x * scale, argv.y * scale);

            if (!el) {
                _log("cannot find image at (" + argv.x + "," + argv.y + "," + scale + ")");
                return;
            }
            
            function _getWXLongPressImageEventConfig() {
                var config = {};
                try {
                    if (typeof window.getWXLongPressImageEventConfig === 'function') {
                        config = window.getWXLongPressImageEventConfig();
                    }
                } catch (e) { }
                return config;
            }
            ...
            _getImageDataAsBase64(el, function (base64Data, img, el) {
                _call('saveImage', {
                    ...
                    'url': img.src,
                    'elementWidth': el.width / scale,
                    'elementHeight': el.height / scale,
                    'elementTop': el.getBoundingClientRect().top / scale,
                    'elementLeft': el.getBoundingClientRect().left / scale,
                    'eventConfigDict': _getWXLongPressImageEventConfig()
                });
            });
        });
    ...
    window.alert = function (msg) {
        if (document.__wxjsjs__isWebviewWillClosed === 'yes' || document.__wxjsjs__isDisableAlertView === 'yes') {
            return;
        }
        return _oldAlert(msg);
    };

    var _oldPrompt = window.prompt;
    window.prompt = function (title, content) {
        if (document.__wxjsjs__isWebviewWillClosed === 'yes' || document.__wxjsjs__isDisableAlertView === 'yes') {
            return;
        }
        return _oldPrompt(title, content);
    };
    ...
    var __WeixinJSBridge = {
        ...
        on: _onfor3rd,
        log: _log,
        _getSelectedText: _getSelectedText,
        // private
    ...
    try {
        ...
        return; //证明已经被hook修改了，直接返回
        //__WeixinJSBridge._handleMessageFromWeixin = _handleMessageFromWeixin;
    }
    ...
    if (true /* __ISWKWEBVIEW */) {
        ...
        _WXJS("document").ready(function () {
            ...
            document.oncopy = function () {
                var txt_cr = _getSelectedText();
                _sendMessage(JSON.stringify({
                    '__onCopy': txt_cr
                }));
                if (typeof (oldCopy) !== "undefined" && oldCopy !== null) {
                    return oldCopy();
                };

            };
            _sendMessage(JSON.stringify({
                '__domReadyNotify': _xxyy
            }));
        });

        //如果是通过pushState和replaceState改变页面，需要将toUrl告诉客户端
        if (history.pushState) {
            var hookPushState = history.pushState
            history.pushState = function (stateObject, title, url) {
                var realUrl = getFullUrl(url)
                    // your code goes here
                _sendMessage(JSON.stringify({
                    '__pageStateChange': realUrl
                }));

                hookPushState.apply(this, arguments)
            }
        }

        if (history.replaceState) {
            var hookReplaceState = history.replaceState
            history.replaceState = function (stateObject, title, url) {
                ...
                hookReplaceState.apply(this, arguments)
            }
        }
    } else {
