          ...
          <template #content="{ row }">
            <div class="expand-wrapper">
              <div v-if="row.status === 'fail'"
                   style="display: inline-block; background-color: rgba(188,188,188,0.16); width: 100%; height: auto; border-radius: 10px">
              ...
              <a-row>
                ...
                <a-button type="primary" size="mini" @click="$refs.xTable.toggleRowExpand(row)"
          ...
          <template #default="{ row }" #content="{row1}">
            <a-tooltip title="点击展开执行详情">
              <a-tag :color="row.status.indexOf('success') !== -1 ? 'green' : 'red'"
                     @click="$refs.xTable.toggleRowExpand(row)">
                {{ suiteSta[row.status] }}
              </a-tag>
            </a-tooltip>
